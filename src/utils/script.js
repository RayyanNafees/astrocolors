...
const getCurrentColor = (index) => {
  return hsvToRgb(currentHue[index], currentSaturation[index], currentValue[index]);
};
...
function attachColorPickerListeners(colorPickers, hexInputs) {
  ...
  const updateColorPickers = () => {
    ...
    for (let i = 0; i < colorPickers.length; i++) {
      ...
      const pickerRect = document.querySelectorAll('.sat-lightness-picker')[i].getBoundingClientRect();
...
const updateColor = (index) => {
  ...
  selectedColor.setAttribute('style', `background-color: rgb(${getCurrentColor(index)})`);

  const currentColor = getCurrentColor(index);
  const calculatedHexValue = rgbToHex(...currentColor);

  if (hexInput.value !== calculatedHexValue && hexInput.dataset.modified !== 'true') {
...
for (let i = 0; i < hueRotationInputs.length; i++) {
  ...
  const handleHueInput = (e) => {
    ...
    hueUnderlays[i].setAttribute('style', `background: rgb(${hsvToRgb(currentHue[i], 1, 1)})`);
    slInput.style.backgroundColor = `rgb(${getCurrentColor(i)})`;
    updateColor(i);
  };
...
const colorSchemes = [
  {
    ...
    generateColors: function() {
      ...
      const firstLightness = isDarkMode ? Math.floor(Math.random() * 11) + 90 : Math.floor(Math.random() * 11);
      const secondLightness = isDarkMode ? Math.floor(Math.random() * 11) : Math.floor(Math.random() * 11) + 90;
      ...
      const primbuttnLightness = secondLightness <= 20 ? Math.floor(Math.random() * 61) + 20 : Math.floor(Math.random() * 61) + 20;
      const accentLightness = isDarkMode ? secbuttnLightness + 40 : secbuttnLightness - 40;
...
function randomizeColors() {
  ...
  if (selectedScheme === 'all') {
    randomColorScheme = colorSchemes[Math.floor(Math.random() * colorSchemes.length)];
  } else {
    randomColorScheme = colorSchemes.find(scheme => scheme.name.toLowerCase().replace(/ /g, '-') === selectedScheme);
  }
  ...
  document.documentElement.style.setProperty('--text', isPrimaryLocked ? primaryColor.value : hexColors[0]);
  document.documentElement.style.setProperty('--bg', isSecondaryLocked ? secondaryColor.value : hexColors[1]);
  document.documentElement.style.setProperty('--primary', isPrimbuttnLocked ? primbuttnColor.value : hexColors[2]);
  document.documentElement.style.setProperty('--secondary', isSecbuttnLocked ? secbuttnColor.value : hexColors[3]);
...
function fillDivsWithShades() {
  ...
  if (primaryHSL.s !== 0) {
    const primaryHueExists = existingHues.some((hue) => Math.abs(hue - primaryHSL.h) < 10);
    if (!primaryHueExists) {
      for (let i = 0; i < numShades; i++) {
        const lightness = (maxLightness / (numShades - 1)) * i;
  ...
  if (secondaryHSL.s !== 0) {
    const secondaryHueExists = existingHues.some((hue) => Math.abs(hue - secondaryHSL.h) < 10);
  ...
  if (accentHSL.s !== 0) {
    const accentHueExists = existingHues.some((hue) => Math.abs(hue - accentHSL.h) < 10);
    if (!accentHueExists) {
      for (let i = 0; i < numShades; i++) {
        const lightness = (maxLightness / (numShades - 1)) * i;
...
downloadButton.addEventListener('click', () => {
  ...
  if (!isValidFileName(fileName)) {
    errorMessage.textContent = 'Uh-oh! File name should include only A-Z, a-z, 0-9, -, _ and .';
  ...
  const paletteFile = new File([paletteBlob], fileName + '-palette.png', { type: 'image/png' });
...
Thanks for using RealtimeColors.com!`;
  ...
  const colorFile = new File([colorBlob], fileName + '-codes.txt', { type: 'text/plain' });
...
function checkContrast() {
  ...
  secbuttnColor.addEventListener('input', debounce(() => {
    const secbuttnColorValue = secbuttnColor.value;
    handleInput(secbuttnColorValue, [...faqQuestions, ...secondaryButtons, ...partTwo, ...secondaryBox]);
  }));
...
function getBrightnessAbs() {
  ...
  updateButtonStyle(primaryButtons, primbuttnColorClass, primbuttnColorValue, primaryBrightness, secondaryBrightness, primaryColorValue);

  // Update Primary Boxes
  updateButtonStyle(primaryBox, primbuttnColorClass, primbuttnColorValue, primaryBrightness, secondaryBrightness, primaryColorValue);

  // Update FAQ questions
  updateButtonStyle(faqQuestions, secbuttnColorClass, secbuttnColorValue, primaryBrightness, secondaryBrightness, primaryColorValue);

  // Update secondary buttons
  updateButtonStyle(secondaryButtons, secbuttnColorClass, secbuttnColorValue, primaryBrightness, secondaryBrightness, primaryColorValue);

  // Update secondary boxes
  updateButtonStyle(secondaryBox, secbuttnColorClass, secbuttnColorValue, primaryBrightness, secondaryBrightness, primaryColorValue);

  // Update part two
  updateButtonStyle(partTwo, secbuttnColorClass, secbuttnColorValue, primaryBrightness, secondaryBrightness, primaryColorValue);

  // Update accent color
  updateAccentColor(accentColorClass, accentColorValue, primaryBrightness, secondaryBrightness, primaryColorValue);
  // Update accent color
  updateButtonStyle(accentBox, accentColorClass, accentColorValue, primaryBrightness, secondaryBrightness, primaryColorValue);
}

function updateButtonStyle(buttons, colorClass, colorValue, primaryBrightness, secondaryBrightness, primaryColorValue) {
  const contrastRatio = getContrastRatio(colorValue, primaryColorValue);
...
function updateAccentColor(colorClass, colorValue, primaryBrightness, secondaryBrightness, primaryColorValue) {
  const contrastRatio = getContrastRatio(colorValue, primaryColorValue);
...
function updateSlug() {
  const colors = [primaryColor, secondaryColor, primbuttnColor, secbuttnColor, accentColor].map(color => {
...
function applyColorsFromSlug() {
  ...
  if (slug) {
    const decodedColors = slug.split('-').map(color => `#${decodeURIComponent(color)}`);
    [primaryColor.value, secondaryColor.value, primbuttnColor.value, secbuttnColor.value, accentColor.value] = decodedColors;
    hexInputs.forEach((input, index) => input.value = decodedColors[index]);

    const cssVariables = ['--text', '--bg', '--primary', '--secondary', '--accent'];
...
[primaryColor, secondaryColor, primbuttnColor, secbuttnColor, accentColor].forEach(color => {
...
function updateColors() {
  ...
  const slug = colors.map(color => encodeURIComponent(color.replace('#', ''))).join('-');
  window.history.replaceState({}, document.title, `?colors=${slug}`);
...
document.addEventListener('keydown', function(event) {
  if ((event.key === 'ArrowLeft' || (event.ctrlKey || event.metaKey) && event.key === 'z')) {
    document.getElementById('undo').click();
  } else if ((event.key === 'ArrowRight' || ((event.ctrlKey || event.metaKey) && event.shiftKey && (event.key === 'z' || event.key === 'Z')))) {
...
function getContrastRatio2(color1, color2) {
  ...
  const contrastRatio2 = (Math.max(luminance1, luminance2) + 0.05) / (Math.min(luminance1, luminance2) + 0.05);
...
function getLuminance2(color) {
  ...
  const luminance = 0.2126 * adjustGamma(red) + 0.7152 * adjustGamma(green) + 0.0722 * adjustGamma(blue);
...
function updateContrastColor(variable, element) {
  ...
  tooltipSpan.innerHTML = `${contrastText} - ${contrastRatio2.toFixed(2)}:1 <br><span style="font-size: 10px; opacity: 60%;">Learn more</span>`;
}
...
'accent': '${accentColorValue}',`;
  ...
  copyButton.addEventListener('click', () => {
    navigator.clipboard.writeText(tcssCode)
      .then(() => {
        copyButton.textContent = 'Copied!';
        setTimeout(() => {
          copyButton.textContent = 'Copy';
...
function generateCSVCodeAndCopy() {
  ...
  document.getElementById('csv-code-without-hash').textContent = csvCodeWithoutHash;
  ...
  copyButtonWithoutHash.addEventListener('click', () => {
    navigator.clipboard.writeText(csvCodeWithoutHash)
      .then(() => {
  ...
  copyButtonWithHash.addEventListener('click', () => {
    navigator.clipboard.writeText(csvCodeWithHash)
      .then(() => {
...
function closeSettingsOnClickOutside(event) {
  const isClickedInsideSettings = Array.from(event.target.classList).some((className) =>
    className.startsWith('color-settings')
  );
...
function toggleTheme() {
  ...
  const targetDivIds = ["randomize", "export", "undo", "redo", "share-link", "prim-cont", "sec-cont", "primbuttn-cont", "secbuttn-cont", "accent-cont"];

  const overlays = [];

  const blockedKeys = ["ArrowRight", "ArrowLeft", " ", "Control", "z", "Shift", "e", "s"]; 
  ...
  setTimeout(() => {
    ...
    const themeContainer = themeDiv.querySelector(".darkorlight-cont");
    const themeWrapper = themeContainer.children[0];
    themeWrapper.style.transform = isDarkMode ? "translateY(0)" : "translateY(-30px)";
    ...
    if (colorParam) {
      ...
      const hexValue0 = setLightness(hexValues[0], lightnessRange0[0], lightnessRange0[1]);
      const hexValue1 = setLightness(hexValues[1], lightnessRange1[0], lightnessRange1[1]);
      const hexValue3 = setLightness(secbuttnColor.value, lightnessRange2[0], lightnessRange2[1]);
      const hexValue4 = setLightness(accentColor.value, lightnessRange3[0], lightnessRange3[1]);
...
function setLightness(hexColor, minLightness, maxLightness) {
  const hslColor = hexToHsl(hexColor);
  const updatedColor = setHslLightness(hslColor, getRandomLightness(minLightness, maxLightness));
  return updatedColor;
}
